    ...
    <option v-for="tokuisaki in tokuisakiList" :key="tokuisaki.tokuisakicd" :value="tokuisaki.tokuisakicd">
     ...
     <!-- ボタン -->
      ...
      <button class="confirmed" @click="achievements">
        <DocumentChecked class="icon" /> 棚卸データ作成
      </button>
    </div>
...
const formattedDate = (() => {
  ...
  try {
       ...
       const response = await axios.post("https://www.hokuohylogi.com/tTanaoroshiCheckT/adjustmentdata", {
...
const tokuisakinm = selectedTokuisaki ? selectedTokuisaki.tokuisakinm : "得意先未設定";
  ...
  const skFileName = `${filters.value.arrivalDate.replace(/-/g, "")}_${tokuisakinm}_棚卸SKDATA.csv`;
