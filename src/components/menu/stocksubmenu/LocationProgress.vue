...
const fetchProgressSummary = async (filterParams) => {
  try {
    ...
    const response = await axios.get("https://www.hokuohylogi.com/tLocationTFinish/progressSummary", {
      params: updated,
    });
...
const fetchProgressDetails = async (filterParams) => {
  try {
    const updated = { ...filterParams, centercd };
    const response = await axios.get("https://www.hokuohylogi.com/tLocationTFinish/progressDetails", {
      params: updated,
    });
    return response.data;
  } catch (error) {
...
const handleDownload = () => {
 ...
 const formattedDate = date?.replace(/[-/]/g, "") || "未選択日";
  ...
  // 進捗区分名の取得
  const denpyokubunLabel = denpyokubunVal === "51"
    ? "未作業"
    : denpyokubunVal === "52"
    ? "移動済"
    : "不明区分";
  ...
  // ヘッダー
  const header = [
    "商品CD", "商品名", "入数", "移動後ケース", "移動後バラ", "移動前ケース","移動前バラ","ロケーション", "サプライヤ様"
  ];
