  ...
  <div class="container">
    ...
    <div class="filter-container">
      <div class="filter-item">
        <label for="arrival-date">調整作業日</label>
        <input
          type="date"
          id="arrival-date"
          class="filter-input"
    ...
    <option v-for="tokuisaki in tokuisakiList" :key="tokuisaki.tokuisakicd" :value="tokuisaki.tokuisakicd">
     ...
     <!-- ボタン -->
      ...
      <button class="confirmed" @click="achievements">
        <DocumentChecked class="icon" /> 調整データ作成
      </button>
    </div>
...
const formattedDate = (() => {
  ...
  try {
    ...
    const response = await axios.post("https://www.hokuohylogi.com/tLocationTFinish/adjustmentdata", {
...
const tokuisakinm = selectedTokuisaki ? selectedTokuisaki.tokuisakinm : "得意先未設定";
  ...
  // CSVファイルのダウンロード処理
  const fileName = `${filters.value.arrivalDate.replace(/-/g, "")}_${tokuisakinm}_調整NKDATA.csv`;
  ...
  const skHeaders = [
    "取込データ区分", "得意先コード", "客先伝票番号", "伝票日付", "納入日", "伝票区分",
    "入出庫先コード", "入出庫先名", "入出庫先部署名", "入出庫先郵便番号", "入出庫先住所１",
    "入出庫先住所２", "入出庫先電話番号", "荷送人コード", "個口数", "車番", "乗務員コード",
    "扱便コード", "摘要１", "摘要２", "摘要３", "フリー項目１", "フリー項目２", "フリー項目３",
    "客先伝票行番号", "商品コード", "ロット１", "ロット２", "ロット３", "ロット４", "ロット５",
  ...
  const skFileName = `${filters.value.arrivalDate.replace(/-/g, "")}_${tokuisakinm}_調整SKDATA.csv`;
