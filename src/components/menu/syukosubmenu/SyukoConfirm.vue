  ...
  <div class="container">
    ...
    <el-dialog v-model="isModalVisible" title="確認" center width="30%">
      <div class="confirmation-box">
        <p>
          <strong>出庫日:</strong>
          <span>{{ filters.arrivalDate || '未設定' }}</span>
        </p>
        <p>
          <strong>得意先:</strong>
          <span>{{ getSelectedTokuisakiName(filters.tokuisaki) || '未設定' }}</span>
        </p>
        <p>上記で出庫確定してよろしいですか？</p>
      </div>
...
<script setup>
  ...
  const confirmEntry = async () => {
    ...
    try {
      ...
      console.error('出庫確定に失敗しました:', error);
      alert('出庫確定に失敗しました。');
    } finally {
  ...
  const handleProgressCheck = async () => {
    ...
    try {
      // バックエンドへデータ送信
      const response = await axios.post('https://www.hokuohylogi.com/shelving/achievements', {
        kisandata: formattedDate, // スラッシュ形式に変換
  ...
  const difference = () => {
    ...
    const headers = ['商品CD', '商品名', '入庫予定数', '入数', '格納総数', '誤差', 'サプライヤ様'];
    ...
    const tokuisakiName =
      ...
      '未設定';
    const fileName = `${formattedDate}_${tokuisakiName}_グロス出し差分.csv`;
