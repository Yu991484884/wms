  ...
  <div class="container">
    ...
    <div class="filter-container">
      ...
      <div class="filter-item">
        ...
        <select id="denpyokubun" class="filter-select" v-model="progressFilters.denpyokubun">
          ...
          <option value="52">検品中</option>
          <option value="53">出荷済</option>
        </select>
      </div>

      <div class="filter-item">
        <label for="batno-code">バッチNo</label>
        <select id="batno" class="filter-select" v-model="progressFilters.batno">
          <option value=""></option>
    ...
    <div class="summary-table-container">
      <el-table :data="summaryTableData" stripe style="width: auto">
        <el-table-column prop="uninspected" label="未作業" width="100">
        ...
        <el-table-column prop="inspected" label="検品中" width="100">
          <template v-slot="scope">
        ...
        <el-table-column prop="stored" label="出荷済" width="100">
          <template v-slot="scope">
...
<script setup>
  ...
  // サマリーテーブルデータ
  const summaryTableData = ref([{ uninspected: '', inspected: '', stored: '' }]);
  ...
  const handleDownload = () => {
    ...
    const tokuisakiName =
      document.querySelector(`#tokuisaki option[value="${filters.value.tokuisaki}"]`)?.innerText ||
      '未設定';
    const progressName =
      document.querySelector(`#denpyokubun option[value="${progressFilters.value.denpyokubun}"]`)
        ?.innerText || '未設定';
  ...
  const handleProgressCheck = async () => {
    try {
      ...
      if (
        !progressFilters.value.denpyokubun ||
        !filters.value.tokuisaki ||
        !filters.value.arrivalDate
      ) {
        alert('検索条件を選択してください');
        return;
      }
      ...
      isEditButtonVisible.value =
        progressFilters.value.denpyokubun === '53' && detailsData.length > 0;
